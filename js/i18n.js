/* ============================================================
   VIKINGPAL — Shared i18n + Language Switcher
   OakDev & AI AB © 2026
   ============================================================ */
'use strict';
...
/* ─── Ember / Particle Canvas (shared legal pages) ─────── */
(function initEmbers() {
  ...
  function mkEmber() {
    ...
    const hue    = gold ? (Math.random() * 20 + 35) : (Math.random() * 20 + 15);
    const sat    = gold ? 80 : 70;
    return {
      x:     Math.random() * W,
      y:     H + Math.random() * 60,
  ...
  function draw(ts) {
    ...
    for (const e of embers) {
      e.x  += e.vx + Math.sin(t * e.freq * 30 + e.phase) * 0.3;
      e.y  += e.vy;
      const a = e.alpha * (0.6 + 0.4 * Math.sin(t * e.freq * 60 + e.phase));
      ctx.beginPath();
  ...
  async function loadTranslations(lang) {
    try {
      ...
      if (lang !== 'en') {
        try {
          const base = document.head.querySelector('meta[name="base-url"]')?.content || '';
          const res  = await fetch(`${base}/locales/en.json`);
          translations = await res.json();
        } catch { translations = {}; }
      } else { translations = {}; }
    }
